...
function main() {
    ...
    webglLessonsUI.setupSlider("#scale", {value: scales[focus_index][0], slide: updateScale(0, 1), min: -5, max: 5, step: 0.01, precision: 2});
    ...
    function updateScale(indexX, indexY) {
        return function(event, ui) {
            scales[focus_index][indexX] = ui.value;
            scales[focus_index][indexY] = ui.value;
            drawScene();
        };
    }
    ...
    canvas.onmousedown = function(event) { 
        for (let i = 0; i < data.length; i++) {
            if (Math.abs(event.clientX - translations[i][0]) <= 50 && Math.abs(event.clientY - translations[i][1]) <= 50) {
                ...
                document.querySelector("#scale > .gman-widget-outer > .gman-widget-slider").value = scales[focus_index][0] * 100
                document.querySelector("#scale > .gman-widget-outer > .gman-widget-value").innerHTML = scales[focus_index][0].toFixed(2)
